      ...
      await handleStreamingResponse(response, responseArea, (data) => {
        ...
        if (data.message?.content) {
          if (data.format === "json" || data.format?.type === "object") {
            try {
              return (
                JSON.stringify(JSON.parse(data.message.content), null, 2) + "\n"
              );
            } catch {
...
// Create Model
document
  .getElementById("createModelForm")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    showSpinner(); // Show spinner before request
    const form = e.target;
...
function updateModelOperationsForm() {
  const action = document.querySelector(
    '#modelOperationsForm [name="action"]',
  ).value;
  ...
  const streamField = document.querySelector(
    '#modelOperationsForm [name="stream"]',
  ).parentElement;
...
document
  .getElementById("modelOperationsForm")
  .addEventListener("submit", async (e) => {
    e.preventDefault();
    showSpinner(); // Show spinner before request
    const form = e.target;
